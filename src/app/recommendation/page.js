...
export default function Home() {
  ...
  const handleGenreClick = (selectedGenre) => {
    setGenre(prev => prev.includes(selectedGenre) ? prev.filter(g => g !== selectedGenre) : [...prev, selectedGenre]);
  };
  ...
  return (
    <main className="bg-black min-h-screen py-8">
      ...
      <section className="container mx-auto px-4 bg-gray-900 p-8 rounded-lg shadow-lg">
        <h2 className="text-3xl text-white font-bold mb-4 text-center">Cari Film Sesukamu!</h2>
        <p className="text-gray-400 mb-8 text-center">Masukkan data supaya kami bisa carikan film yang sesuai untukmu!</p>

        <div className="mb-6">
          <h3 className="text-xl text-white font-bold mb-2">Genre</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {['Aksi', 'Komedi', 'Drama', 'Sains Fiksi', 'Fantasi', 'Romantis', 'Thriller', 'Misteri', 'Horror', 'Petualangan', 'Dokumentari', 'Musikal'].map(genreItem => (
              <button
                key={genreItem}
                onClick={() => handleGenreClick(genreItem)}
                className={`py-2 px-4 rounded border ${genre.includes(genreItem) ? 'bg-gray-700' : 'bg-gray-800'} text-white hover:bg-gray-700 transition`}
              >
                {genreItem}
              </button>
            ))}
        ...
        <div className="mb-8">
          <h3 className="text-xl text-white font-bold mb-2">Film yang Kamu Nikmati</h3>
          <input
            type="text"
